...
def main():
    ...
    # Configurar nome de usuário e email Git
    print("Configurando usuário Git...")
    username = "jeanrrf"
    
    # Solicitar email se necessário
    email = input("Digite seu email para o Git (ou pressione Enter para pular): ").strip()
    if not email:
        email = f"{username}@users.noreply.github.com"
    
    # Configurar nome de usuário
    stdout, stderr, rc = run_command(f"git config --global user.name {username}")
    if rc != 0:
        print(f"Erro ao configurar nome de usuário: {stderr}")
    else:
        print(f"Nome de usuário Git configurado: {username}")
    
    # Configurar email
    stdout, stderr, rc = run_command(f"git config --global user.email {email}")
    if rc != 0:
